...
class AmericanOddsTest {
    ...
    fun `should convert positive american odds to decimal`() {
        assertThat(AmericanOdds(100).toDecimal().percentage.value)
            .isEqualByComparingTo("2.0")

        assertThat(AmericanOdds(150).toDecimal().percentage.value)
            .isEqualByComparingTo("2.5")

        assertThat(AmericanOdds(200).toDecimal().percentage.value)
    ...
    fun `should convert negative american odds to decimal`() {
        ...
        assertThat(AmericanOdds(-400).toDecimal().percentage.value)
            .isEqualByComparingTo("1.25")
    }

    @Test
    fun `should convert minus one hundred ten to approximately one point nine zero nine`() {
        assertThat(AmericanOdds(-110).toDecimal().percentage.value)
